...
enum {TYPE_NUM=1,TYPE_OPER_0,TYPE_OPER_1,TYPE_OPER_MAX,TYPE_LEFT,TYPE_RIGHT,TYPE_MAX};
...
expr_elem* find_a_numb(expr_elem &expr_head, char **Estr_p, int *expr_level){
    ...
    *expr_level=char_Analysis(expr_head.oper);
    ...
    if(Lexpr_lvl==TYPE_NUM || Lexpr_lvl>=*expr_level){
    ...
    if(Rexpr_lvl==TYPE_NUM || Rexpr_lvl>=*expr_level){
...
void expr_printf(expr_elem &expr_head){
    ...
    if(Lexpr_lvl==TYPE_NUM || Lexpr_lvl>=expr_lvl){
    ...
    //input right-expr to the Estr_p.
    if(Rexpr_lvl==TYPE_NUM || Rexpr_lvl>=expr_lvl){
